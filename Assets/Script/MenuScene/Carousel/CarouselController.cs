...
namespace Carousel
{
    ...
    public class CarouselController<T> : MonoBehaviour
    {
        ...
        {
            var selectedItem = _carouselItems[GetCarouselIndex(_currentIndex)].Data;
            _onItemSelected?.Invoke(selectedItem);

            if (selectedItem is LocationData locationData)
            {
                if (!string.IsNullOrEmpty(locationData.sceneName))
                {
                    UnityEngine.SceneManagement.SceneManager.LoadScene(locationData.sceneName);
                    // LoadingManager.Instance.LoadScene(locationData.sceneName);
                }
                else
                {
                    Debug.LogWarning("Scene name is empty for the selected location!");
                }
            }
        }
